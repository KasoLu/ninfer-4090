...
// RK6V4E8: warp-wide 24-bit quad gather and write. Four consecutive lanes hold
// one 24-bit quad (3 bytes); every lane converges on each full-mask shuffle
// read from an explicit source lane, and the 4-lane leader writes the packed
// d0 quad at `row` and the d1 quad at `row + 24`. All lanes of the warp must
// call this together. The explicit-source full-mask gather (not a
// __shfl_down_sync over a per-quad sub-mask) is the only mask form this
// codebase relies on: the sub-mask variant made the group's non-leader lanes
// read source lanes outside their mask — undefined behavior that deadlocked
// sm_89 (4090 hang 2026-09-04).
