...
// Prefill/Decode rates, draft acceptance, cache hits and phase durations. The shape matches
// llama.cpp's server `timings` JSON so existing metric parsers work unchanged.
void add_timings(Json& payload, const CompletionUsage& usage) {
    ...
    if (usage.ttft_seconds > 0.0) { timings["ttft_ms"] = usage.ttft_seconds * 1000.0; }
    if (usage.cached_tokens > 0) { timings["cache_n"] = usage.cached_tokens; }
