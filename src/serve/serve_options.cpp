...
std::string serve_usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "[--default-max-tokens N] [--default-thinking-budget N] "
            "[--vision] [--vision-max-tokens N] [--chat-template v22_4] [--no-cuda-graph] "
            "[--no-prefix-reuse] "
           ...
           "       --vision enables media and loads the fixed Vision GPU allocations\n"
            ...
            "       --chat-template selects a registered chat template by name (default: the "
            "chat template embedded in the artifact)\n"
...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--host") {
            ...
            options.chat_template_name = require_value("--chat-template");
