  ...
  Value pop(const Value& index) {
    if (is_array()) {
      ...
      if (index.is_null()) {
        ...
        const auto size = static_cast<int>(array_->size());
        if (i < -size || i >= size)
  ...
  bool contains(const Value & value) const {
    ...
    if (array_) {
      for (const auto& item : *array_) {
        if (item == value) return true;  // local fix (upstream PR #42 review): falsy items (0/False/"") must satisfy `in`
      }
      return false;
    } else if (object_) {
...
public:
    ...
    void do_render(std::ostringstream & out, const std::shared_ptr<Context> & context) const override {
      ...
      std::function<void(Value&)> visit = [&](Value& iter) {
          ...
          if (!iter.is_null()) {
            const Value & source = iter;  // local fix (upstream PR #42 review): recursive loop() calls must walk the provided sub-iterable, not the outer iterable
            if (!source.is_iterable()) {
              throw std::runtime_error("For loop iterable must be iterable: " + source.dump());
            }
            source.for_each([&](Value & item) {
