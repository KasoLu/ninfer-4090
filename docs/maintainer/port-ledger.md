...
|---|---|---|
| `05a88712`, transient admission shortfall bricks the executor | class confirmed, instance unproven | OPEN, highest stakes. Reviewed 2026-09-01: the FAILURE CLASS is real here - `fail_all_locked` (`engine_core.h:2026`) latches `failed_`, the worker returns, and both submit paths then throw Unavailable; the worker's catch-all at ~2135 turns ANY exception escaping an execution action into that permanent state. The admission path throws `std::logic_error` in at least five places, and the closest analogue to their shortfall is `"isolated-feasible request is blocked in an idle Engine"` (thrown when the FIFO head is not Ready and no lane is active) - which is the exact situation their fix converts into a per-request ContextLengthExceeded. Note our pending loop ALREADY does the right thing for its neighbours: a failed `ensure_base_plan` and a `PermanentlyInfeasible` inspection each fail only that request. What is NOT established is whether a concurrent restore or snapshot save can actually drive us into that state, which needs a repro rather than a reading. Their fix is in `concurrent_executor.h`, which this tree does not have; our admission lives in `engine_core.h` and throws `std::logic_error` in several places inside `admit_planned_request`, with the same `worker_loop` catch-all and latched `failed_`. The symptom they describe - worker thread exits, every later request 503, `/health` still ok - is worth a targeted review here, especially now that restores run in production and their trigger is a concurrent restore or snapshot save consuming pages before admission |
